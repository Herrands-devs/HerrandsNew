...
import { ActivityIndicator, Image, Platform, RefreshControl, TouchableOpacity } from 'react-native'
    ...
    >
      ...
      errandsData.map((item , index) => {
        ...
        <TouchableOpacity 
          ...
          onPress={() => navigation.navigate("InProgressBoard" , {data : item})}
          key={index}
        >
          <View className="flex gap-2 flex-row justify-between items-center">
            <Text className="font-montserratRegular text-[#C6C6C6]">{new Date(item.created).getDate()+"/"+new Date(item.created).getMonth()+"/"+new Date(item.created).getFullYear()}</Text>
            <View className="bg-[#34A853] py-[7px] px-[18px] rounded-full">
              <Text className="font-montserratRegular text-white">In Progress</Text>
            </View>
          </View>

          <View className="flex flex-col gap-2">
            <Text className="text-[#000E23] text-[20px]">{item.subtype.name}</Text>
            <Text className="text-[#C6C6C6] text-[14px] font-montserratRegular">Duration : {item.estimated_drop_off_time}</Text>
          </View>
        ...
        <View className="flex flex-row w-full h-full justify-between">
          <TabView
            ...
            renderScene={({route}) => {
              switch (route.key)  {
                case 'first' : 
                 return FirstRoute(navigation , isLoading, errandsData , refreshing);
                case 'second' : 
                 return SecondRoute(navigation , isFetching , completedErrands , refreshing);
                default : 
                 return null
              }
            }}
