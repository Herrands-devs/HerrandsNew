...
import { KeyboardAvoidingView, ScrollView, Text, TouchableOpacity } from 'react-native'
   ...
   return (
      <View className="relative">
         <View className="w-full h-screen absolute">
            <Image source={require('../../../../assets/chatbg.png')} className="w-full h-full"/>
         </View>
         <SafeAreaView className="h-screen">
            <View className="relative z-10 p-3 flex gap-5 bg-[#1f2227c8]  flex-row">
               <TouchableOpacity onPress={() => navigation.goBack()} className="h-[48px] w-[18px] fle justify-center items-center">
            ...
            >
               ...
               {/* Left chat */}
            {chat && chat.message_set.map((item , index) => {
                  return item.sender === userId ? 
                  (
                  <View className="w-full flex flex-row">
                     <View className="bg-[#CCE0FD] max-w-[70%] p-2 flex rounded-br-sm rounded-md" key={index}>
                        <Text>{item.text}</Text>
                        <View className="flex w-full justify-end flex-row h-[20px] items-end">
                        <Text className="text-xs font-montserratMedium">{new Date(item.timestamp).getHours().toString().padStart(2, '0')}:{new Date(item.timestamp).getMinutes().toString().padStart(2, '0')} {new Date(item.timestamp).getMinutes().toString().padStart(2, '0') > 12 ? 'PM' : 'AM'}  </Text>
                        </View>
                     </View>
                  </View>
                  )
                  :
                  (
                  <View className="w-full flex flex-row justify-end" key={index}>
                     <View className="bg-[#0066F5] max-w-[70%] p-2 flex rounded-br-sm rounded-md">
                        <Text className="text-white">{item.text}</Text>
                        <View className="flex w-full justify-end flex-row h-[20px] items-end">
                           <Text className="text-white text-xs font-montserratMedium">{new Date(item.timestamp).getHours().toString().padStart(2, '0')}:{new Date(item.timestamp).getMinutes().toString().padStart(2, '0')} {new Date(item.timestamp).getMinutes().toString().padStart(2, '0') > 12 ? 'PM' : 'AM'}  </Text>
                        </View>
                     </View>
                  </View>
                  )
