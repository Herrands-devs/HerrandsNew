...
const CompleteScreen = ({ navigation }) => {
  ...
  const handleSubmit = () => {
    setLoading(true);
    axios
    .put(`${API_URl}/accounts/update/data/`, 
      data,
      {
        headers: {
    ...
    <KeyboardAvoidingContainer>
      <View style={styles.container} className="p-3 gap-2">
        ...
        <View className="relative flex flex-col w-full gap-2 items-center">
          <TouchableOpacity onPress={handleDocumentSelection} className="flex gap-3 py-2 items-center">
            <View className="w-[119px] h-[119px] rounded-full flex justify-center items-center bg-[#D9D9D9]">
              {!filePreview ?
               ...
               :
               <Image source={filePreview ? {uri : filePreview} : require('../../../../assets/icons/camera.png')}  className="w-full  h-full rounded-full object-fit"/>
              }
            </View>
          ...
          <View className="flex w-full">
            <PrimaryInput
              ...
              onChangeText={(text) =>
                setData({ ...data, arrival_speed:  text })
              }
            />
          </View>
          <View className="flex w-full">
