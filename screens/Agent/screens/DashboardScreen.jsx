...
const DashboardScreen = ({ navigation }) => {
   const { Agent, isToken , setAgent ,createErrandSent , receiveErrand , acceptedErrand } = useContext(GlobalContext)
      ...
      <View>
         <View className="relative h-full bg-red">
            <View className="absolute z-30 w-[38px] flex justify-between items-center h-[82px] rounded-[50px] bg-white shadow-lg top-[40%] right-5">
               ...
               :
               <TouchableOpacity
                  className="h-1/2 w-full bg-slate-200 rounded-b-[40px] flex justify-center items-center"
               >
                  <Feather name="minus" size={24} color="black" />
               </TouchableOpacity>
               }
            ...
            <View style={styles.container}>
               <MapView 
                  style={styles.map}
                  initialRegion={{
                     latitude: location ? parseFloat(location.coords.latitude): 9.0820,
                     longitude: location ? parseFloat(location.coords.longitude) : 8.6753,
            ...
            {!isEmpty(receiveErrand) &&  isEmpty(acceptedErrand)&& <IncomeOrder data={receiveErrand}/> }
            {isEmpty(receiveErrand) && !isEmpty(acceptedErrand) && <InProgress data={acceptedErrand}/>}
